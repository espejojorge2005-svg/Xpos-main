        ...
        {zones.length > 0 && (
          <div className="flex items-center gap-2 overflow-x-auto pb-1 pt-0.5 scrollbar-none">
            ...
            {zones.map((zone) => {
              ...
              const occupiedInZone = zone.tables.filter(t => t.status !== 'FREE').length;
              ...
              return (
                ...
                >
                  ...
                  {occupiedInZone > 0 && (
                    <span className="w-2 h-2 rounded-full bg-rose-500 animate-pulse" title={`${occupiedInZone} mesas ocupadas`} />
                  )}
                </button>
              );
            })}
            ...
            <section key={zone.id} className="bg-white p-3 md:p-5 rounded-3xl shadow-sm border border-slate-100">
              <div className="flex justify-between items-center mb-4 md:mb-5">
                <div className="flex items-center gap-3 border-l-4 border-emerald-500 pl-3">
                  ...
                  {zone.tables.filter(t => t.status !== 'FREE').length > 0 && (
                    <span className="text-xs font-bold px-2 py-0.5 bg-rose-50 text-rose-600 rounded-md border border-rose-100 flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" />
                      {zone.tables.filter(t => t.status !== 'FREE').length} ocupadas
                    </span>
                  )}
                </div>
              </div>
